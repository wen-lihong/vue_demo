          ...
          <li v-for = "item in cartList">
            <div class="cart-tab-1">
              <div class="cart-item-check">
                <a  class="checkbox-btn item-check-btn" v-bind:class="{'checked':item.checked==1}" @click ="oncheck('checked',item)">
              ...
              <div class="item-quantity">
                <div class="select-self select-self-open">
                  <div class="select-self-area">
                    <a class="input-sub" @click ="oncheck('minu',item)">-</a>
                    <span class="select-ipt">{{item.productNum}}</span>
                    <a class="input-add" @click ="oncheck('add',item)">+</a>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="cart-tab-5">
              <div class="cart-item-opration">
                <a href="javascript:;" class="item-edit-btn" @click="editCartModel(item.productId)">
      ...
      <div class="cart-foot-inner">
        <div class="cart-foot-l">
          <div class="item-all-check">
            <a @click="oncheckAll">
                  <span class="checkbox-btn item-check-btn" v-bind:class="{'checked':checkedAll==1}">
        ...
        <div class="cart-foot-r">
          ...
          <div class="btn-wrap">
            <a class="btn btn--red" v-bind:class="{'btn--dis':checkeCount==0}"
            @click="checkOut">
            Checkout</a>
...
<Model v-bind:mdShow="cartModelFlag" v-on:closs = 'closs'>
	<p slot ="message">确定要删除该商品吗？</p>
	<div slot = "btnGroup">
		<a href="javascript:;" class="btn btn--m" @click = "editCart()">确定</a>
		<a href="javascript:;" class="btn btn--m" @click = "cartModelFlag=false">取消</a>
	</div>
</Model>
</div>		
</template>
...
<script type="text/javascript">
  ...
  import axios from 'axios'
   export default{
    ...
    computed:{
      checkedAll(){
         if(this.checkeCount==this.cartList.length){
          return '1'
         }else{
          return '0'
         }
    ...
    methods:{
    	//加载购物车页面初始数据
    	init(){
    		axios.get("/users/cart").then((respones)=>{
        ...
        editCart(productId){
    		...
    		axios.post("/users/editCart",{productId:productid}).then((respones)=>{
    			let res = respones.data;
    			if(res.status == 0){
    			this.cartList.forEach((item,index)=>{
                  if(item.productId==this.productId){
                  	console.log(index)
                  	// this.cartList=this.cartList.splice(this.cartList.findIndex(i => i.productId === productId), 1)
        ...
        //check
        oncheck(flag,item){
        	if(flag=="minu" && item.productNum>=2){
         ...
         axios.post("/users/checked",{productNum:item.productNum,
          productId:item.productId,
          checked:item.checked}).then((respones)=>{
            let num =0;
          if(flag=="minu" && item.productNum>=2){ 
           num =-1;
          }else if(flag=="add"){
        ...
        //oncheckAll
        oncheckAll(){
          let checkAllflag = this.checkedAll
          checkAllflag=checkAllflag==1?0:1;
            this.cartList.forEach((item)=>{
              item.checked= checkAllflag;
              })
          
         axios.post("/users/checkedAll",{checkedAll:checkAllflag,
         }).then((respones)=>{
              
          }) 
        },
        checkOut(){
          if(this.checkeCount>0){
            this.$router.push({
              path:"/address"
              })
          }
