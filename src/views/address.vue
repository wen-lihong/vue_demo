  ...
  <div class="container">
    <div class="checkout-addr">
      ...
      <div class="addr-list-wrap">
        <div class="addr-list">
          <ul>
            <li v-for="(item,index) in addressListFiter" v-bind:class="{'check':
            checkIndex==index}" @click="checkIndex=index;confirmIndex=item.addressId">
              <dl>
                <dt>{{item.userName}}</dt>
              ...
              <div class="addr-opration addr-del">
                <a href="javascript:;" class="addr-del-btn" @click="delAddressConform(item.addressId)">
                  <svg class="icon icon-del"><use xlink:href="#icon-del"></use></svg>
                </a>
              </div>
              <div class="addr-opration addr-set-default">
                <a href="javascript:;" class="addr-set-default-btn"
                @click="setDefault(item.addressId)" v-if="!item.isDefault"><i>Set default</i></a>
              </div>
      ...
      <div class="next-btn-wrap">
        <router-link class="btn btn--m btn--red" v-bind:to="{path:'/orderConfirm',query:{'addressId':confirmIndex}}">Next</router-link>
      </div>
    </div>
  </div>
</div>
<model :mdShow="ismdShow" @closs="closeModel">
   <p slot="message">你确定要删除这个地址</p>	
   <div slot="btnGroup">
...
<script type="text/javascript">
  ...
  import axios from 'axios'
   export default{
    ...
    methods:{
    	init(){
    		axios.get("/users/addressList").then((respones)=>{
       ...
       setDefault(addressId){
    		axios.post("/users/setDefault",{addressId:addressId}).then((respones)=>{
    			let res = respones.data;
    			if(res.status == 0){
    			this.init()
    			}
      ...
      delAddressConform(addressId){
      this.ismdShow=true
      this.addressId =addressId   
  },
     delAddress(){
      this.ismdShow=true
    		axios.post("/users/delAddress",{addressId:this.addressId}).then((respones)=>{
    			let res = respones.data;
    			if(res.status == 0){
    			this.ismdShow=false
    			this.init()
