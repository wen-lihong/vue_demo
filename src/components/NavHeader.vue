            ...
            <div class="md-content">
              <div class="confirm-tips">
                <div class="error-wrap" >
                  <span class="error error-show" v-show = "loginTip">{{loginTip}}</span>
                </div>
                <ul>
    ...
    data(){
     ...
     methods:{
      ...
      login(){
        ...
        axios.post("/users/login",params).then((respons)=>{
      ...
      //检测登录
      relogin(){
        axios.get("/users/relogin").then((respons)=>{
          let res = respons.data;
          if(res.status == 0){
           this.$store.commit('updateUserInfo',res.result.userName);
           this.getcartCount()
      ...
      //登出功能
      loginOut(){
        axios.post("/users/loginOut").then((respons)=>{
           let res = respons.data;
           if(res.status == 0){
      ...
      //获取购物车数量
      getcartCount(){
        axios.get("/users/cartCount").then((respons)=>{
          let res = respons.data;
          if(res.status == 0){
